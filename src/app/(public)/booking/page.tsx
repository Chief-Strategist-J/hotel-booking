...
export default async function BookingPage({ searchParams }: { searchParams: { roomId?: string; checkIn?: string; checkOut?: string } }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <h1 className="font-serif text-3xl font-bold text-primary mb-8">Complete Your Booking</h1>
      ...
      {!available ? (
        ...
        <div className="grid md:grid-cols-5 gap-10">
          ...
          <div className="md:col-span-2">
            <div className="bg-gray-50 rounded-xl overflow-hidden border sticky top-20">
              <div className="relative h-48">
                {primary ? <Image src={primary.imageUrl} alt={room.name} fill className="object-cover" /> : <div className="w-full h-full bg-gray-200 flex items-center justify-center text-gray-400">No image</div>}
              </div>
              <div className="p-5 text-sm text-gray-700 space-y-2">
                <p className="font-serif text-xl font-bold text-primary">{room.name}</p>
